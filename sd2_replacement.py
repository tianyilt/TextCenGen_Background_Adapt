...
parser.add_argument('--path', type=str,default='/dataset/prompt.txt', help='The path for prompt')
parser.add_argument('--start', type=int,default=0, help='The start line number for prompt')
parser.add_argument('--end', type=int,default=1000, help='The end line number for prompt')

args = parser.parse_args()
model_id = "checkpoints/sd2.0"  # SD2.1 基础版（512x512）
frames = [
    ...
    for line_number, line in enumerate(file, start=1):
        for i in range(len(frames)):
            if start <= line_number <= end:
                ...
                folder_path = ('{}/output_sd2_replacement0124/{}/{}_{}').format(current_directory, outdir,prompt[:50], current_time)
